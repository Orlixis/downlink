/// Embedded Apple Liquid Glass Mobile Web Companion for iOS, Android, and Tablets
pub const MOBILE_APP_HTML: &str = r###"<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, "SF Pro Display", "SF Pro Text", "Segoe UI", Roboto, sans-serif; -webkit-tap-highlight-color: transparent; }
    body { background-color: #09090b; color: #f4f4f5; min-height: 100vh; display: flex; flex-direction: column; align-items: center; justify-content: flex-start; padding: env(safe-area-inset-top, 24px) 16px env(safe-area-inset-bottom, 24px) 16px; background-image: radial-gradient(circle at 50% 0%, rgba(6, 182, 212, 0.15), transparent 45%), radial-gradient(circle at 80% 80%, rgba(59, 130, 246, 0.08), transparent 50%); background-attachment: fixed; }
    .container { width: 100%; max-width: 440px; display: flex; flex-direction: column; gap: 16px; margin: 0 auto; }
    .glass-card { background: rgba(24, 24, 27, 0.65); backdrop-filter: blur(28px); -webkit-backdrop-filter: blur(28px); border: 1px solid rgba(255, 255, 255, 0.08); border-radius: 22px; box-shadow: 0 20px 40px -15px rgba(0, 0, 0, 0.6), inset 0 1px 0 rgba(255, 255, 255, 0.1); }
    header { display: flex; align-items: center; justify-content: space-between; padding: 12px 4px 8px; }
    .brand { display: flex; align-items: center; gap: 10px; font-weight: 700; font-size: 19px; letter-spacing: -0.02em; color: #ffffff; }
    .status-pill { display: inline-flex; align-items: center; gap: 6px; padding: 5px 12px; border-radius: 20px; font-size: 11px; font-weight: 600; background: rgba(16, 185, 129, 0.12); color: #34d399; border: 1px solid rgba(16, 185, 129, 0.25); }
    .dot { width: 6px; height: 6px; border-radius: 50%; background: #34d399; box-shadow: 0 0 8px #34d399; animation: pulse 2s infinite; }
    @keyframes pulse { 0%, 100% { opacity: 1; transform: scale(1); } 50% { opacity: 0.5; transform: scale(0.85); } }
    .input-card { padding: 20px; }
    .card-header { display: flex; align-items: center; justify-content: space-between; margin-bottom: 10px; }
    .label { font-size: 12px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.06em; color: #a1a1aa; }
    .btn-paste { background: rgba(255, 255, 255, 0.06); border: 1px solid rgba(255, 255, 255, 0.1); color: #38bdf8; font-size: 11px; font-weight: 600; padding: 4px 10px; border-radius: 8px; cursor: pointer; transition: all 0.2s; }
    .btn-paste:active { transform: scale(0.95); background: rgba(56, 189, 248, 0.2); }
    textarea { width: 100%; min-height: 110px; background: rgba(9, 9, 11, 0.7); border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 14px; padding: 14px; color: #fff; font-size: 14px; line-height: 1.4; resize: none; outline: none; transition: all 0.2s; }
    textarea:focus { border-color: #38bdf8; box-shadow: 0 0 0 3px rgba(56, 189, 248, 0.15); }
    .presets-label { font-size: 11px; font-weight: 600; color: #71717a; text-transform: uppercase; letter-spacing: 0.05em; margin: 16px 0 8px; }
    .presets { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 8px; }
    .preset-btn { padding: 10px 4px; border-radius: 12px; border: 1px solid rgba(255, 255, 255, 0.08); background: rgba(255, 255, 255, 0.03); color: #a1a1aa; font-size: 12px; font-weight: 600; text-align: center; cursor: pointer; transition: all 0.2s; }
    .preset-btn.active { background: rgba(56, 189, 248, 0.15); border-color: rgba(56, 189, 248, 0.4); color: #38bdf8; box-shadow: 0 0 12px rgba(56, 189, 248, 0.2); }
    .btn-send { width: 100%; padding: 15px; border-radius: 15px; background: linear-gradient(135deg, #38bdf8, #2563eb); color: #fff; font-size: 15px; font-weight: 700; border: none; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 8px; margin-top: 16px; box-shadow: 0 10px 24px -6px rgba(37, 99, 235, 0.6); transition: all 0.2s; }
    .btn-send:active { transform: scale(0.98); opacity: 0.9; }
    .btn-send:disabled { opacity: 0.5; cursor: not-allowed; }
    .toast { position: fixed; bottom: 32px; left: 50%; transform: translateX(-50%) translateY(100px); background: rgba(24, 24, 27, 0.95); backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px); border: 1px solid rgba(56, 189, 248, 0.3); color: #fff; padding: 14px 24px; border-radius: 30px; font-size: 13px; font-weight: 600; opacity: 0; transition: all 0.35s cubic-bezier(0.16, 1, 0.3, 1); box-shadow: 0 16px 32px rgba(0,0,0,0.6); z-index: 1000; pointer-events: none; display: flex; align-items: center; gap: 8px; }
    .toast.show { transform: translateX(-50%) translateY(0); opacity: 1; }
    .tip { text-align: center; font-size: 11px; color: #71717a; margin-top: 8px; padding: 12px; line-height: 1.5; }
    .tip strong { color: #a1a1aa; }
  </style>
</head>
<body>
  <div class="container">
    ...
    <main>
      <div class="glass-card input-card">
        <div class="card-header">
          <label class="label">Beam Link to Desktop</label>
          <button id="pasteBtn" class="btn-paste" type="button">Paste</button>
        </div>
        <textarea id="urlInput" placeholder="Paste YouTube, TikTok, M3U8, or Magnet link..." spellcheck="false"></textarea>
        
        <div class="presets-label">Quality Preset</div>
        <div class="presets">
          <button class="preset-btn active" data-preset="recommended_best" type="button">Best 4K/8K</button>
          <button class="preset-btn" data-preset="video_1080p" type="button">1080p FHD</button>
          <button class="preset-btn" data-preset="audio_mp3" type="button">Audio MP3</button>
        </div>
      ...
      <div class="tip">
        <strong>Pro Tip:</strong> Add this page to your iPhone or Android Home Screen to beam media directly from the native <strong>Share Sheet</strong>.
      </div>
    </main>
  </div>
  ...
  <div id="toast" class="toast">
    ...
    <span id="toastMsg">Added to Downlink Queue</span>
  </div>

  <script>
    let currentPreset = 'recommended_best';
